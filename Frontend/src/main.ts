...
bootstrapApplication(AppComponent, {
  providers: [
    provideHttpClient(withInterceptorsFromDi()),  // Lägg till denna för att hantera autentisering via HTTP-interceptors
    importProvidersFrom(FormsModule, ReactiveFormsModule),
    provideRouter([
      { path: 'login', loadComponent: () => import('./app/components/login/login.component').then(m => m.LoginComponent) }, // Inloggningssidan
      ...
      { path: '', redirectTo: '/login', pathMatch: 'full' }  // Om ingen route anges, omdirigera till inloggningssidan
    ])
  ]
});
